...
добавлена система боссов: каждый 30-й противник является боссом, у него ×3 здоровья и 30 секунд на убийство. за победу над боссом выдаётся сфера босса. при провале id противника откатывается к позиции после предыдущего босса, временно снимаются награды в сферах и очках перековки. если окно неактивно или идёт просчёт оффлайн-прогресса — босс пропускается с тем же штрафом

добавлена кузня: улучшение за сферы босса, каждый уровень повышает максимальный префикс атрибутов на 10% (базовый уровень 1 = максимум 100%)

система перековки переработана: вместо фиксации атрибутов — выбор конкретного стата для замены. стоимость зависит от редкости: обычный 1оп, редкий 2оп, эпический 3оп, легендарный 4оп

добавлен эффектный слой на canvas: визуальные эффекты при крите (золотая звезда), срабатывании подготовленного (фиолетовое кольцо) и двойном ударе (бирюзовый импульс)

полоска здоровья переработана: плавный цветовой градиент, отображение числовых значений, вторая полоска с задержкой 300мс
